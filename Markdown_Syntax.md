...
另外，如果在` ``` `后面加上代码块中语言的标识(代码文件后缀或文件名，如：cpp、c、sh、vim、py、ruby ... 等)，一些Markdown解析器会依据所用语言进行语法高亮：
...
_Note: Table syntax is only supported on server side._

Markdown 可以用HTML的`<table>`标签来定义表格，也可以采用如下Markdown语法：
...
_Note: Index syntax is only supported on server side._

Markdown 可使用`[TOC]`根据层级标题自动生成目录：
...
_注意_：只有当所有标题都是按照Markdown语法书写时，`[TOC]`才有效。

<h3 id="delete">删除线</h3>

_Note: Table syntax is only supported on server side._

Markdown 可用连续的两个`~`为包围内的内容添加删除线效果：
